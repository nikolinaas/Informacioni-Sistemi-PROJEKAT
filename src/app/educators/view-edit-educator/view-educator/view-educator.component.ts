...
export class ViewEducatorComponent {
  ...
  openMedicalFile(){
    this.educatorsService.getMedicalFile(this.id).subscribe((response: ArrayBuffer) => {
      this.regenerateFile(response);
    });
  }
  openHygieneFile(){
    this.educatorsService.getHygieneFile(this.id).subscribe((response: ArrayBuffer) => {
      this.regenerateFile(response);
    });
  }
