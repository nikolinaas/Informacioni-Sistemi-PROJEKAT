...
import { Educator } from 'src/app/model/educator.model';
...
export class CreateEducatorDialogComponent {
  ...
  createEducator(){
    if(this.educator.name != '' && this.educator.surname != '' ){ //ovdje vidjeti ovu provjeru
      const date: string='';
      const data = {
      ...
      this.educatorService.createEducator(data).subscribe(
        (response: any) => {
          if (response.status == 201) {
            this.snackBar.open('Uspjesno ste kreirali nalog vaspitača! ', undefined, {
              duration: 2000,
            });
        ...
        () => {
          this.snackBar.open(
            'Nije moguće kreirati nalog vasptača!'+date,
            undefined,
            {
              duration: 2000,
            }
  ...
  onHygieneFileSelected(event: any) {
    ...
    if (file1) {
      console.log(file1 , "usao u higijenski");
